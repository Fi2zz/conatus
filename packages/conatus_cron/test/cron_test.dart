...
({CronService service, CronStorage storage, Directory dir, List<String> warnings})
    buildService({
...
void main() {
  group('任务 CRUD', () {
    test('动态任务：缺省 id 生成 + 会话绑定，显式 sessionId 优先', () {
      ...
      final CronTaskView explicit = built.service.addDynamicTask(
          <String, Object?>{'prompt': 'pong', 'every': 600, 'sessionId': 'sess-2'},
          callerSessionId: 'sess-1');
    ...
    test('重复 id 与非法输入抛 CronException', () {
      ...
      expect(
        ...
        throwsA(isA<CronException>()
            .having((CronException e) => e.code, 'code', CronErrorCode.duplicateId)),
      );
      expect(
        () => built.service.addDynamicTask(
            <String, Object?>{'prompt': 'a', 'every': 5}),
        throwsA(isA<CronException>()
            .having((CronException e) => e.code, 'code', CronErrorCode.invalidTask)),
      );
    });
    ...
    test('update：仅 prompt 原地更新；规则替换清空运行戳', () {
      ...
      final CronTaskView replaced = built.service.updateDynamicTask(
          't', <String, Object?>{'daily': '09:30'});
    ...
    test('update：patch 携带两个规则或非法规则时拒绝', () {
      ...
      expect(
        () => built.service
            .updateDynamicTask('t', <String, Object?>{'every': 5}),
        throwsA(isA<CronException>()
            .having((CronException e) => e.code, 'code', CronErrorCode.invalidTask)),
      );
    });
    ...
    test('config 任务保护：update / remove 拒绝，add 撞 id 报重复', () {
      ...
      expect(
        ...
        throwsA(isA<CronException>()
            .having((CronException e) => e.code, 'code', CronErrorCode.configTask)),
      );
      expect(
    ...
    test('enabledOverride 与声明值一致时归 null', () {
      final built = buildService(configTasks: <Map<String, Object?>>[
        <String, Object?>{'id': 'off', 'prompt': 'a', 'daily': '08:00', 'enabled': false},
      ]);
      built.service.addDynamicTask(
  ...
  group('持久化 round-trip', () {
    test('新实例读旧文件：运行戳与覆盖恢复，已消费槽位不重发', () {
      ...
      expect(loop.lastRunAt!.millisecondsSinceEpoch, base.millisecondsSinceEpoch);
      ...
      expect(dueSlot(loop, base.add(const Duration(seconds: 600)), restored.startedAt),
          isNull,
          reason: '停用任务不触发');
      loop.enabledOverride = null;
      expect(
          dueSlot(loop, base.add(const Duration(seconds: 599)), restored.startedAt),
          isNull,
          reason: 'lastRunAt 已消费该时段');
      expect(
          dueSlot(loop, base.add(const Duration(seconds: 600)), restored.startedAt),
          isNotNull);
      expect(dueSlot(once, base.add(const Duration(days: 1)), restored.startedAt),
          isNull,
          reason: 'at 任务消费后不再触发');
    });
    ...
    test('历史封顶 500，seq 连续，最新在前', () {
      ...
      final File historyFile =
          File('${built.dir.path}/cron-history.jsonl');
