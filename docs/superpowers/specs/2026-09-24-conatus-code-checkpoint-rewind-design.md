...
| 存储压缩 | 2026-09-24 追加：每个检查点**打包成单个 `.gz` 归档文件**（`<会话>/<turn>.gz`，自定义容器：gzip(清单JSON+条目流)，`dart:io` 内置 GZipCodec，零新依赖），**不保留目录结构**、内容非明文；读时自动回退旧版目录树检查点 | 用户反馈明文副本令人不安，且要求「整个项目压缩成一个文件而不是保留结构输出」；单文件同时减小磁盘占用 |
